...
<body>
<div class="shell">
 <div class="header"><div><h1>Nano Lead-Acid Charger</h1><div class="sub">ESP8266 direct hotspot control panel</div></div><div id="topLink" class="status-pill warn">NANO --</div></div>
 <div class="tabs">
 ...
 <section id="tab-firmware" class="tab-page"><div class="grid">
  <div class="card wide"><div class="label">ESP8266 monitor firmware</div><div class="instruction">Choose a NanoLeadAcidChargerSM .bin. It is written to the ESP8266 OTA slot and the ESP restarts automatically.</div><input id="espFile" class="file-input" type="file" accept=".bin,application/octet-stream"><div class="controls"><button id="espFlash" class="flashBtn" onclick="uploadFirmware('esp')">FLASH ESP8266 .BIN</button></div></div>
  <div class="card wide"><div class="label">Arduino Nano charger firmware</div><div class="instruction">Choose a NanoLeadAcidCharger .bin. The ESP sends STOP, resets the Nano into its bootloader, writes and verifies the firmware, then restarts the Nano.</div><div class="foot warn">Nano web flashing requires the separate programming wiring: ESP D1/GPIO5 -> Nano D0/RX; Nano D1/TX -> divider -> ESP D2/GPIO4; ESP D7/GPIO13 -> reset MOSFET; common ground. Maximum Nano .bin size is 30720 bytes.</div><input id="nanoFile" class="file-input" type="file" accept=".bin,application/octet-stream"><div class="controls"><button id="nanoFlash" class="flashBtn" onclick="uploadFirmware('nano')">FLASH NANO .BIN</button></div></div>
