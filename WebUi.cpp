...
body{margin:0;background:#0b0f13;color:#eef2f5}
.shell{width:min(980px,100%);margin:0 auto;padding:16px}
.header{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:14px}
h1{font-size:1.55rem;margin:0 0 4px}.sub{color:#8d9aa6;font-size:.9rem}.status-pill{border:1px solid #34404b;background:#151b21;border-radius:999px;padding:8px 12px;font-weight:700;font-size:.82rem;white-space:nowrap}
.tabs{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin-bottom:14px;position:sticky;top:0;z-index:5;background:#0b0f13;padding:8px 0}
.tab-btn{border:1px solid #34404b;background:#151b21;color:#aeb8c1;border-radius:10px;padding:11px 8px;margin:0;font-size:.9rem;font-weight:700;cursor:pointer}.tab-btn.active{background:#2a3641;color:#fff;border-color:#536475}
.tab-page{display:none}.tab-page.active{display:block}
.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}
.card{background:#151b21;border:1px solid #2c3741;border-radius:13px;padding:14px}.wide{grid-column:1/-1}
.label{font-size:.75rem;color:#8796a3;text-transform:uppercase;letter-spacing:.075em}.value{font-size:1.55rem;font-weight:700;margin-top:5px;word-break:break-word}.small{font-size:1rem}
.ok{color:#73dc8d}.bad{color:#ff7b7b}.warn{color:#ffd36f}
.controls{display:flex;flex-wrap:wrap;gap:7px;margin-top:10px}
button{border:0;border-radius:9px;padding:11px 15px;margin:0;font-size:.94rem;font-weight:700;cursor:pointer}button:disabled{opacity:.45;cursor:default}
.primary,#auto,#syncStart,#vcalStart,#vcalSubmit,#termSend{background:#4fc36a;color:#07120a}.danger,#stop,#syncStop,#vcalStop{background:#df5757;color:white}.secondary,#refresh,#firmware,#termClear{background:#36424d;color:white}.quick{background:#202a33;color:#d8e0e6;border:1px solid #394651;padding:8px 10px;font-size:.82rem}
input{box-sizing:border-box;border:1px solid #46515d;border-radius:9px;background:#0c1116;color:#fff;padding:11px;font-size:1rem}
.foot{color:#84919d;font-size:.8rem;margin-top:9px}.row{margin-top:8px;font-size:.95rem}.row b{color:#eef2f5}
.instruction{margin-top:10px;padding:10px;border-radius:8px;background:#0d1217;line-height:1.45}
.code{background:#080b0e;border:1px solid #343d46;border-radius:8px;padding:10px;margin-top:10px;white-space:pre-wrap;word-break:break-word;color:#aee9ba}
pre{white-space:pre-wrap;word-break:break-word;margin:0;color:#aeb9c2}
.terminal{height:330px;overflow:auto;background:#050708;border:1px solid #2f3a43;border-radius:10px;padding:12px;color:#b7f5c5;font:13px/1.45 monospace;white-space:pre-wrap;word-break:break-word}
.terminal-row{display:grid;grid-template-columns:1fr auto;gap:8px;margin-top:10px}.terminal-row input{width:100%;font-family:monospace}.quick-row{display:flex;flex-wrap:wrap;gap:6px;margin-top:10px}.hint{color:#7f8b96;font-size:.8rem;line-height:1.45;margin-top:8px}
.system-line{display:flex;justify-content:space-between;gap:10px;padding:8px 0;border-bottom:1px solid #26313a}.system-line:last-child{border-bottom:0}.system-line span:first-child{color:#8796a3}.system-line span:last-child{text-align:right;font-weight:700}
@media(max-width:620px){.grid{grid-template-columns:1fr}.tabs{grid-template-columns:repeat(2,1fr)}.header{align-items:center}.value{font-size:1.35rem}.terminal{height:290px}.terminal-row{grid-template-columns:1fr}.terminal-row button{width:100%}}
</style>
</head>
<body>
<div class="shell">
  <div class="header">
    <div><h1>Nano Lead-Acid Charger</h1><div class="sub">ESP8266 direct hotspot control panel</div></div>
    <div id="topLink" class="status-pill warn">NANO --</div>
  </div>
  ...
  <section id="tab-dashboard" class="tab-page active">
    <div class="grid">
      ...
      <div class="card wide">
        <div class="label">Charging controls</div>
        <div class="controls">
          <button id="auto" onclick="cmd('AUTO')">AUTO</button>
          <button id="stop" onclick="cmd('STOP')">STOP CHARGING</button>
          <button id="refresh" onclick="refreshNow()">REFRESH</button>
        </div>
        <div id="cmdResult" class="foot">Ready.</div>
      </div>
    </div>
  </section>

  <section id="tab-calibration" class="tab-page">
    <div class="grid">
      <div class="card wide">
        ...
        <div class="foot">Point 1 is measured with charging held OFF. It then returns to AUTO, waits for real charging and a temperature rise, records Point 2, then Point 3. Each point averages 60 readings.</div>
      </div>
  ...
  <section id="tab-terminal" class="tab-page">
    <div class="card">
      <div class="label">Nano command terminal</div>
      <div class="foot">Type a Nano command below and press SEND or Enter.</div>
      <div id="terminalOutput" class="terminal">Nano terminal ready. Type HELP to list commands.</div>
      <div class="terminal-row">
        <input id="terminalInput" type="text" autocomplete="off" autocapitalize="characters" spellcheck="false" placeholder="HELP" maxlength="47">
        <button id="termSend" onclick="sendTerminal()">SEND</button>
      </div>
      <div class="quick-row">
        <button class="quick" onclick="terminalQuick('HELP')">HELP</button>
        <button class="quick" onclick="terminalQuick('PING')">PING</button>
        <button class="quick" onclick="terminalQuick('STATUS')">STATUS</button>
        <button class="quick" onclick="terminalQuick('BATTERY')">BATTERY</button>
        <button class="quick" onclick="terminalQuick('TEMP')">TEMP</button>
        <button class="quick" onclick="terminalQuick('STATE')">STATE</button>
        <button class="quick" onclick="terminalQuick('AUTO')">AUTO</button>
        <button class="quick" onclick="terminalQuick('STOP')">STOP</button>
        <button id="termClear" onclick="clearTerminal()">CLEAR</button>
      </div>
      <div class="hint">Accepted commands: PING, STATUS, BATTERY, TEMP, STATE, TSYNC START/STATUS/STOP, VCAL START/STATUS/STOP, VCAL SAMPLE &lt;volts&gt;, AUTO, STOP and HELP.</div>
    </div>
  </section>
  ...
  <section id="tab-system" class="tab-page">
    <div class="grid">
      <div class="card wide">
        ...
        <div class="system-line"><span>Nano UART</span><span id="link">--</span></div>
        <div class="system-line"><span>Status age</span><span id="age">--</span></div>
        <div class="system-line"><span>Network mode</span><span class="ok">HOTSPOT</span></div>
        <div class="system-line"><span>Hotspot IP</span><span id="ips">--</span></div>
      </div>
      <div class="card wide"><div class="label">Last Nano line</div><pre id="raw">--</pre></div>
      <div class="card wide">
        <div class="label">Firmware</div>
        <div class="instruction">Open the firmware updater to flash ESP8266 or Nano firmware .bin files.</div>
        <div class="controls"><button id="firmware" onclick="location.href='/firmware'">OPEN FIRMWARE UPDATE</button></div>
      </div>
    </div>
  </section>
...
let lastNanoLineSeen='';
let terminalLines=['Nano terminal ready. Type HELP to list commands.'];
...
function showTab(name){
 ...
 if(name==='terminal')setTimeout(()=>document.getElementById('terminalInput').focus(),50);
}

function renderTerminal(){
 const out=document.getElementById('terminalOutput');
 out.textContent=terminalLines.join('\n');
 out.scrollTop=out.scrollHeight;
}
function appendTerminal(line){
 if(!line)return;
 terminalLines.push(line);
 if(terminalLines.length>80)terminalLines.splice(0,terminalLines.length-80);
 renderTerminal();
}
...
async function refreshNow(){
 try{
  ...
  const top=document.getElementById('topLink');top.textContent=d.nanoConnected?'NANO CONNECTED':'NANO OFFLINE';top.className='status-pill '+(d.nanoConnected?'ok':'bad');
  ...
  if(tp==='WAIT_POINT2')ti='Point 1 complete. Charger is back in AUTO. Waiting for real CHARGING and about a 2 °C temperature rise.';
  ...
  if(tp==='POINT3')ti='Point 3 of 3: collecting the final 60 samples while charging.';
  if(tp==='COMPLETE')ti='TEMP SYNC complete. Copy the four code lines below into PinsAndConfig.h and reflash the Nano. Then move the external sensor back to the battery.';
  ...
  if(vp==='INPUT3'){vi='Reading 3 of 3: enter the final multimeter voltage.';wantsInput=true;}
  ...
  const ln=document.getElementById('link');ln.textContent=d.nanoConnected?'CONNECTED':'OFFLINE';ln.className=d.nanoConnected?'ok':'bad';
  setText('age',d.statusAgeMs===null?'No status received':d.statusAgeMs+' ms');
  setText('ips',d.hotspotIp);setText('raw',d.lastNanoLine||'--');

  if(d.lastNanoLine&&d.lastNanoLine!==lastNanoLineSeen){lastNanoLineSeen=d.lastNanoLine;appendTerminal('< '+d.lastNanoLine)}
 }catch(e){
  const top=document.getElementById('topLink');top.textContent='WEB UPDATE ERROR';top.className='status-pill bad';
  const ln=document.getElementById('link');ln.textContent='WEB UPDATE ERROR';ln.className='bad';setText('age',String(e));
 }
}

async function cmd(c,terminalEcho=false){
 c=String(c||'').trim();if(!c)return false;
 const o=document.getElementById('cmdResult');o.textContent='Sending '+c+'...';
 if(terminalEcho)appendTerminal('> '+c.toUpperCase());
 try{
  const r=await fetch('/api/command?cmd='+encodeURIComponent(c),{method:'POST',cache:'no-store'});
  const d=await r.json();
  if(d.ok){o.textContent='Sent: '+d.command;if(terminalEcho)appendTerminal('[ESP] sent '+d.command);setTimeout(refreshNow,150);setTimeout(refreshNow,400);setTimeout(refreshNow,800);return true;}
  o.textContent='Error: '+d.error;if(terminalEcho)appendTerminal('[ESP] ERROR '+d.error);return false;
 }catch(e){o.textContent='Command failed: '+e;if(terminalEcho)appendTerminal('[ESP] command failed: '+e);return false;}
}

async function sendTerminal(){
 const input=document.getElementById('terminalInput');
 const c=input.value.trim();if(!c)return;
 input.value='';await cmd(c,true);input.focus();
}
function terminalQuick(c){document.getElementById('terminalInput').value=c;sendTerminal()}
document.getElementById('terminalInput').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();sendTerminal()}});
